...
const LEVELS = ["TRACE", "DEBUG", "INFO ", "WARN ", "ERROR", "MUTE"]
const TRACE = 0
const DEBUG = 1
const INFO  = 2
const WARN = 3
...
func trace(message, module = "main"):
  """Log a message in the given module with level TRACE."""
  put(TRACE, message, module)
...
func load_config(configfile = default_configfile_path):
  ...
  if config.get_value(PLUGIN_NAME, "modules"):
    for module_cfg in config.get_value(PLUGIN_NAME, "modules"):
      var module = Module.new(module_cfg["name"], module_cfg["output_level"],   module_cfg["output_strategies"], get_logfile(default_logfile_path))
